...
namespace The_Long_Dark_Save_Editor_2
{
    public class GameSave
    {
        ...
        public string path;
        public string SlotJson => dynamicSlotData.Serialize();
        public string BootJson => dynamicBoot.Serialize();
        public string GlobalJson => dynamicGlobal.Serialize();
        public string[] SectionKeys => SlotData.m_Dict?.Keys.OrderBy(x => x).ToArray() ?? Array.Empty<string>();
        ...
        public void ReplaceRawJson(string slotJson, string bootJson, string globalJson)
        {
            ...
            if (!string.IsNullOrWhiteSpace(bootJson))
            {
                dynamicBoot = new DynamicSerializable<BootSaveGameFormat>(bootJson);
                OriginalRegion = Boot.m_SceneName.Value;
            }
            ...
            {
                var extractedBootJson = EncryptString.Decompress(Convert.FromBase64String(SlotData.m_Dict["boot"]));
                dynamicBoot = new DynamicSerializable<BootSaveGameFormat>(extractedBootJson);
                OriginalRegion = Boot.m_SceneName.Value;
            }
            ...
            {
                var extractedGlobalJson = EncryptString.Decompress(Convert.FromBase64String(SlotData.m_Dict["global"]));
                dynamicGlobal = new DynamicSerializable<GlobalSaveGameFormat>(extractedGlobalJson);
            }
